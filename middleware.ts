...
export async function middleware(req: NextRequest) {
  ...
  const rootDomain = process.env.NEXT_PUBLIC_ROOT_DOMAIN ?? 'qizora.com';
  ...
  if (path.startsWith('/host') && user) {
    const { data: org } = await supabase
      .from('organizations')
      .select('subscription_status, paid_until, trial_ends_at')
      .maybeSingle();
    if (org && !canHostLive(org.subscription_status, org.paid_until, org.trial_ends_at)) {
      const url = req.nextUrl.clone();
